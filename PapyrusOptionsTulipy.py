...
def calculate_technical_indicators(historical_data):
    ...
    # Calculate RSI
    rsi = ti.rsi(close_prices, period=14)[-1]

    # Calculate Bollinger Bands
    upper_bb, middle_bb, lower_bb = ti.bbands(close_prices, period=20)

    # Calculate SMA (5 days)
    sma_5 = ti.sma(close_prices, period=5)[-1]
...
# Function to calculate technical indicators using Tulipy
def calculate_technical_indicators(historical_data):
    close_prices = np.array(historical_data['chart']['result'][0]['indicators']['quote'][0]['close'])
    high_prices = np.array(historical_data['chart']['result'][0]['indicators']['quote'][0]['high'])
    low_prices = np.array(historical_data['chart']['result'][0]['indicators']['quote'][0]['low'])
    ...
    # Calculate RSI
    rsi = tulipy.rsi(close_prices, period=14)[-1]

    # Calculate Bollinger Bands
    upper_bb, middle_bb, lower_bb = tulipy.bbands(close_prices, period=20)

    # Calculate SMA (5 days)
    sma_5 = tulipy.sma(close_prices, period=5)[-1]
